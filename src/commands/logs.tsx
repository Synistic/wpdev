...
export const description = "Show logs for a WordPress environment";
...
export default function Logs({ args: [name] }: Props) {
	...
	useEffect(() => {
		async function run() {
			...
			await execStream(["docker", "compose", "logs", "-f"], { cwd: siteDir });
		}
		run();
	}, [name]);
